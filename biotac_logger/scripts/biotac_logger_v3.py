...
class BIOTAC():
  ...
  def get_centered_data(self,data):
    centered_data = SignedBioTacData()
    centered_data.bt_position = data.bt_data.bt_position
    centered_data.bt_serial = data.bt_data.bt_serial
    centered_data.tdc_data = data.bt_data.tdc_data - self.confirmed_tdc_mu
    centered_data.tac_data = data.bt_data.tac_data - self.confirmed_tac_mu
    centered_data.pdc_data  = data.bt_data.pdc_data - self.confirmed_pdc_mu
    centered_data.pac_data = np.array(data.bt_data.pac_data) - self.confirmed_pac_mu
    centered_data.electrode_data = np.array(data.bt_data.electrode_data) - self.confirmed_electrode_mu
      ...
      if time.time() >= self.end_time:
        ...
        if self.calibration_data_count == 1:
          for i in range(self.n_biotacs):
            self.biotacs[i].pdc_mu_t = data.bt_data[i].pdc_data
            self.biotacs[i].pdc_var_t = 0.0
            self.biotacs[i].electrode_mu_t = data.bt_data[i].electrode_data
            self.biotacs[i].tac_mu_t = data.bt_data[i].tac_data
            self.biotacs[i].tdc_mu_t = data.bt_data[i].tdc_data
            self.biotacs[i].pac_mu_t = data.bt_data[i].pac_data
        else: #online mean and var update formula
          for i in range(self.n_biotacs):
            self.biotacs[i].pdc_mu_t = (self.calibration_data_count - 1.0)/self.calibration_data_count * float(self.pdc_mu_t_1[0]) + 1.0 / self.calibration_data_count * float(data.bt_data[0].pdc_data)
            self.biotacs[i].pdc_var_t = (self.pdc_var_t_1[0]*(self.calibration_data_count - 2.0) + (data.bt_data[0].pdc_data - self.pdc_mu_t_1[0])*(data.bt_data[0].pdc_data - self.pdc_mu_t[0]))/(self.calibration_data_count - 1.0)
            #self.pdc_mu_t_1 = self.pdc_mu_t
            #self.pdc_var_t_1 = self.pdc_var_t
            self.biotacs[i].electrode_mu_t[0] = (self.calibration_data_count - 1.0)/self.calibration_data_count * self.electrode_mu_t_1[0] + 1.0 / self.calibration_data_count * np.array(data.bt_data[0].electrode_data)
            self.electrode_mu_t[1] = (self.calibration_data_count - 1.0)/self.calibration_data_count * self.electrode_mu_t_1[1] + 1.0 / self.calibration_data_count * np.array(data.bt_data[1].electrode_data)
            #self.electrode_mu_t_1 = self.electrode_mu_t
            self.biotacs[i].tac_mu_t[0] = (self.calibration_data_count - 1.0)/self.calibration_data_count * self.tac_mu_t_1[0] + 1.0 / self.calibration_data_count * data.bt_data[0].tac_data
            #self.tac_mu_t_1 = self.tac_mu_t
            self.biotacs[i].tdc_mu_t[0] = (self.calibration_data_count - 1.0)/self.calibration_data_count * self.tdc_mu_t_1[0] + 1.0 / self.calibration_data_count * data.bt_data[0].tdc_data
            #self.tdc_mu_t_1 = self.tdc_mu_t
            self.biotacs[i].pac_mu_t[0] = (self.calibration_data_count - 1.0)/self.calibration_data_count * self.pac_mu_t_1[0] + 1.0 / self.calibration_data_count * np.array(data.bt_data[0].pac_data)
            #self.pac_mu_t_1 = self.pac_mu_t
            print("Calibrating, sample variance: "+str(self.pdc_var_t_1))
